import React from 'react'
import NavBar4 from "../components/NavBar4"
import bg from "../assets/images/depositphotos_11514732-Group-of-professional-female-chefs.jpg"
import { Link } from 'react-router-dom'
import { User, Armchair, Printer } from 'lucide-react';
import chef1 from "../assets/images/Image2.png"
import Footer1 from '../components/Footer1';

const TeamDetails = () => {
  return (
    <div>
      <div>
      <NavBar4/>

      <div className="relative w-full h-[500px] overflow-hidden">
      <div className="absolute inset-0 bg-cover bg-center transition-all duration-700" style={{ backgroundImage: `url(${bg})` }}>
        
        <div className="absolute inset-0 bg-black/50" /></div>
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-white text-center px-4 space-y-4">
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold">OUR CHEF</h1>
          <div className='flex font-bold gap-2'>
            <Link to ="/"> <p>HOME / </p> </Link>
            <Link to="/team"><span>CHEF DETAILS</span></Link>
            </div>

            <h2 className="mt-20 text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-9xl font-bold text-gray-700">TEAM EXPERT</h2>

          
        </div>
        
        
    </div>
      
    </div>

    <div className='W-full mt-20 pb-16'>
      <div className="flex flex-col md:flex-row-reverse gap-6 items-center max-w-6xl mx-auto bg-gray-100 p-14">
      
      
      <div className="flex-1 w-full">
        <img
          src={chef1}
          alt=""
          className="w-full h-full object-cover"
        />
      </div>

      
      <div className="flex-1 w-full space-y-6">
        <h1 className="font-bold text-4xl mt-10">KUNMAN TUMAN</h1>
        <h2 className="text-orange-700 font-bold">Founder & CEO</h2>
        <p className="text-gray-500 text-sm leading-relaxed">
          Sem consequat mauris conubia inceptos nostra rutrum morbi sagittis pulvinar,
          commodo curabitur maecenas fermentum magna tempus nisi ullamcorper,
          ante auctor magnis pretium eu lectus euismod platea.
          Congue sociosqu nostra est urna pellentesque placerat euismod bibendum.
        </p>

        <div className="mt-10 overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr>
                <th className="border-t border-b border-r border-gray-300 px-4 py-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center">
                      <User className="text-orange-700 fill-orange-700" />
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Experience</p>
                      <p className="font-bold">More Than 10 Years</p>
                    </div>
                  </div>
                </th>
                <th className="border-t border-b border-gray-300 px-4 py-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center">
                      <Armchair className="text-orange-700 fill-orange-700" />
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Email</p>
                      <p className="font-bold">info@agency.com</p>
                    </div>
                  </div>
                </th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="border-b border-r border-gray-300 px-4 py-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center">
                      <User className="text-orange-700 fill-orange-700" />
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Phone</p>
                      <p className="font-bold">+(256) 58621-69581</p>
                    </div>
                  </div>
                </td>
                <td className="border-b border-gray-300 px-4 py-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center">
                      <Printer className="text-orange-700 fill-orange-700" />
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Fax</p>
                      <p className="font-bold">+2568145632</p>
                    </div>
                  </div>
                </td>
              </tr>
            </tbody>
          </table>
          <button className='bg-orange-700 mt-10 text-white py-3 px-6 font-semibold'>CONTACT ME</button>
        </div>
      </div>
    </div>
    </div>

    <div className='w-full bg-gray-400 h-auto text-white p-16 mt-16'>
  <div className='max-w-6xl mx-auto flex flex-row items-center justify-between'>
    <div className='flex flex-col'>
      <h1 className='font-bold'>BOOK A TABLE FOR YOU AND YOUR FAMILY MEMBERS</h1>
      <p className='text-4xl font-bold'>NEED A TABLE ON COFFEE HOUSE</p>

    </div>
    <button className='border border-white px-6 py-2 text-sm'>
      BOOK A TABLE
    </button>

  </div>


</div>



        <div>
      <Footer1/>


     </div>

    <div className="w-full bg-black py-6 text-white text-sm">
  <div className="w-full max-w-6xl mx-auto flex flex-col gap-4 px-4">
    <hr className="w-full border-gray-700" />

    <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-center">
      <p className="w-full md:w-auto">
        © {new Date().getFullYear()} Delish. All Rights Reserved.
      </p>

      <ul className="flex flex-col sm:flex-row items-center gap-2 sm:gap-6">
        <li className="cursor-pointer">Terms of Use</li>
        <li className="cursor-pointer">Privacy Policy</li>
      </ul>
    </div>
  </div>
</div>




    </div>
      
    
  )
}

export default TeamDetails
